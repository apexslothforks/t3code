...
function isStalePendingRequestDetail(detail: string | undefined): boolean {
  if (!detail) {
    return false;
  }
  return (
    ...
    detail.includes("Provider session exited before response could be submitted.")
  );
}
...
export function derivePendingApprovals(
  ...
  for (const activity of ordered) {
    ...
    if (
      ...
      requestId &&
      isStalePendingRequestDetail(detail)
    ) {
      openByRequestId.delete(requestId);
      continue;
    }

...
describe("OrchestrationReactor", () => {
  ...
  it("starts provider ingestion, provider command, checkpoint, and scheduled dispatch reactors", async () => {
    ...
    runtime = ManagedRuntime.make(
      Layer.effect(OrchestrationReactor, makeOrchestrationReactor).pipe(
        ...
        Layer.provideMerge(
          Layer.succeed(ScheduledDispatchReactor, {
            start: Effect.sync(() => {
              started.push("scheduled-dispatch-reactor");
            }),
          }),
        ),
      ),
